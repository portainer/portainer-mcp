...
The container defaults to HTTP transport bound on `0.0.0.0:17717`, so it's
...
Clients connect to `http://<host>:17717/mcp` with
`Authorization: Bearer <token>`. Example for Claude Code:

```bash
claude mcp add portainer --transport http http://<host>:17717/mcp \
...
- Forward to the container on `127.0.0.1:17717` or a private network.
...
[`configuration.md`](configuration.md). The image overrides three defaults:
